...
const PaidCourses = () => {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        const paidCourses = data.courses.filter(course => course.course_type === "paid");
  ...
  const handlePurchase = async (contentCode) => {
    ...
    try {
      ...
      formData.append("subscription_content", contentCode);
      ...
      if (data.status === "success") {
        if (data.subscription_code) {
          ...
          fetchCourses(); // إعادة تحميل الكورسات بعد الاشتراك
  ...
  const handlePaymentErrors = (data) => {
    switch (data.reason) {
      ...
      case "insufficient_balance":
        toast.error("الرصيد غير كافٍ لإتمام الشراء.");
        break;
      default:
  ...
  const checkPaymentStatus = async (invoiceId) => {
    ...
    try {
      const response = await fetch(
        `https://apis.mr-biology.com/student/payments/checkPayment?invoice_id=${invoiceId}`
      );
      const data = await response.json();
      ...
      if (data.status === "success") {
        toast.success("تم التحقق من الدفع وتفعيل الاشتراك بنجاح!");
        fetchCourses();
      } else if (data.status === "error") {
        switch (data.reason) {
          case "not_found":
            toast.error("لم يتم العثور على الفاتورة أو تم التحقق منها مسبقاً.");
            break;
          case "missing_parameter":
            toast.error("معرف الفاتورة مفقود.");
            break;
          case "verification_failed":
            toast.error("فشل التحقق من بيانات الفاتورة.");
            break;
          case "database_error":
  ...
  // التحقق تلقائيًا لو فيه invoice_id في الرابط
  useEffect(() => {
    const queryParams = new URLSearchParams(window.location.search);
    const invoiceId = queryParams.get("invoice_id");
    if (invoiceId) checkPaymentStatus(invoiceId);
  }, []);
          ...
          {paginatedCourses.map((course, i) => (
            ...
            >
              ...
              <div className="space-y-6 text-center p-4">
                ...
                >
                  شراء {course.course_price} جنيه <Play />
                </Button>
                <Button
                  variant="outline"
                ...
                {/* زر التحقق من الدفع */}
                {course.payment_invoice_id && (
                  <Button
                    variant="outline"
                    className="w-full mt-2"
                    onClick={() => checkPaymentStatus(course.payment_invoice_id)}
                  >
                    تحقق من الدفع
                  </Button>
                )}
